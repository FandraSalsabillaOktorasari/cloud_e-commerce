...
@section('title', 'Add Product — Admin TechStore')
@section('page-title', 'Add New Product')
@section('page-subtitle', 'Create a new product listing')

@section('content')
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" id="create-product-form">
        @csrf
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full px-4 py-3 bg-surface-900 border border-white/5 rounded-xl text-white focus:outline-none focus:border-primary-500/30 focus:ring-2 focus:ring-primary-500/10 transition-all">
                            @error('name') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="brand" class="block text-sm font-medium text-surface-200 mb-1.5">Brand *</label>
                            <input type="text" name="brand" id="brand" value="{{ old('brand') }}" required class="w-full px-4 py-3 bg-surface-900 border border-white/5 rounded-xl text-white focus:outline-none focus:border-primary-500/30 focus:ring-2 focus:ring-primary-500/10 transition-all">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" class="w-full px-4 py-3 bg-surface-900 border border-white/5 rounded-xl text-white focus:outline-none focus:border-primary-500/30 focus:ring-2 focus:ring-primary-500/10 transition-all">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" required class="w-full px-4 py-3 bg-surface-900 border border-white/5 rounded-xl text-white focus:outline-none focus:border-primary-500/30 focus:ring-2 focus:ring-primary-500/10 transition-all resize-none">{{ old('description') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="stock" id="stock" value="{{ old('stock', 0) }}" required min="0" class="w-full px-4 py-3 bg-surface-900 border border-white/5 rounded-xl text-white focus:outline-none focus:border-primary-500/30 focus:ring-2 focus:ring-primary-500/10 transition-all">
                            @error('stock') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div class="flex items-end pb-1">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" name="is_trending" value="1" {{ old('is_trending') ? 'checked' : '' }} class="w-5 h-5 rounded border-white/10 bg-surface-900 text-primary-600 focus:ring-primary-500/20">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach(['Processor', 'GPU', 'RAM', 'Storage', 'Display'] as $spec)
                        <div>
                            ...
                            <input type="text" name="specifications[{{ $spec }}]" id="spec-{{ Str::slug($spec) }}" value="{{ old("specifications.$spec") }}" class="w-full px-4 py-3 bg-surface-900 border border-white/5 rounded-xl text-white focus:outline-none focus:border-primary-500/30 focus:ring-2 focus:ring-primary-500/10 transition-all" placeholder="e.g. {{ $spec === 'Processor' ? 'Intel Core i7-14700K' : ($spec === 'GPU' ? 'NVIDIA RTX 4070' : ($spec === 'RAM' ? '16GB DDR5' : ($spec === 'Storage' ? '1TB NVMe SSD' : '15.6" FHD IPS'))) }}">
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="bg-surface-800/50 border border-white/5 rounded-2xl p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="image" class="block text-sm font-medium text-surface-200 mb-1.5">Primary Image</label>
                    ...
                    <div>
                        <label for="images" class="block text-sm font-medium text-surface-200 mb-1.5">Additional Images</label>
